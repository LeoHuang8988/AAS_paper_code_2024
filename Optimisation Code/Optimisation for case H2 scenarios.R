...
#================H2 case BASE1 (+1.224% ERP; allocation range is [0,1]) =========================#
...
returnData <- read.csv("Ret_2_asset_alpha1.csv", header = T, stringsAsFactors = F)[-1]
...
save(data_vfi_t7, file = paste0("mgmt_H2_rho_",rho, "salary", salary, "_base1_v5.1.RData"))##
write.csv(data_vfi_t7, paste0("mgmt_H2_rho_",rho, "salary", salary, "_base1_v5.1.csv"), row.names = F)##

###########################################################################################################################################
#================H2 case base 2 (+1.224% ERP; RANGE (-0.32,1.32)) =========================#
...
save(data_vfi_t7, file = paste0("mgmt_H2_rho_",rho, "salary", salary, "_base2_v5.1.RData"))##
write.csv(data_vfi_t7, paste0("mgmt_H2_rho_",rho, "salary", salary, "_base2_v5.1.csv"), row.names = F)##

###########################################################################################################################################

#================H2 case base 3 (+1.224% ERP  RANGE(-0.16,1.16)) =========================#
...
save(data_vfi_t7, file = paste0("mgmt_H2_rho_",rho, "salary", salary, "_base3_v5.1.RData"))##
write.csv(data_vfi_t7, paste0("mgmt_H2_rho_",rho, "salary", salary, "_base3_v5.1.csv"), row.names = F)##


###########################################################################################################################################

#================H2 case sens1_1 (+1.224% ERP; 55k salary) =========================#
...
save(data_vfi_t7, file = paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens1_1_v5.1.RData"))##
write.csv(data_vfi_t7, paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens1_1_v5.1.csv"), row.names = F)##

###########################################################################################################################################

#================H2 case sens1_2 (+1.224% ERP; 85k salary) =========================#
...
save(data_vfi_t7, file = paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens1_2_v5.1.RData"))##
write.csv(data_vfi_t7, paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens1_2_v5.1.csv"), row.names = F)##

###########################################################################################################################################

#================H2 case sens2_1 (+1.224% ERP; rho = 3) =========================#
...
save(data_vfi_t7, file = paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens2_1_v5.1.RData"))##
write.csv(data_vfi_t7, paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens2_1_v5.1.csv"), row.names = F)##

###########################################################################################################################################

#================H2 case sens2_2 (+1.224% ERP; rho = 5) =========================#
...
save(data_vfi_t7, file = paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens2_2_v5.1.RData"))##
write.csv(data_vfi_t7, paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens2_2_v5.1.csv"), row.names = F)##
...
#================H2 case sens4_1 (+1.224% ERP; then -1% ERP) =========================#
...
#importing historical return data
returnData <- read.csv("Ret_2_asset_alpha1.csv", header = T, stringsAsFactors = F)[-1]
itRetA <- as.numeric(returnData$DE) - 0.01
itRetB <- as.numeric(returnData$Cash)
itRetProb <- rep(1 / nrow(returnData), nrow(returnData))
...
result_xx <- foreach(y = 1:length(balances), .combine = rbind, .packages = "schumaker") %dopar% {
  #result_xx <- for(y in 1:length(balances)){#, .combine = rbind, .packages = "schumaker") %dopar% {  
  #cat(paste("balance", y, "in progress \n"))
...
save(data_vfi_t7, file = paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens4_1_v5.1.RData"))##
write.csv(data_vfi_t7, paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens4_1_v5.1.csv"), row.names = F)##

###########################################################################################################################################

#================H2 case sens4_2 (+1.224% ERP; then +1% ERP) =========================#
...
save(data_vfi_t7, file = paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens4_2_v5.1.RData"))##
write.csv(data_vfi_t7, paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens4_2_v5.1.csv"), row.names = F)##

###########################################################################################################################################
#================H2 case sens4_3 (-5.424% ERP; worse alpha scenario) =========================#
...
#importing historical return data
returnData <- read.csv("Ret_2_asset_sens3.csv", header = T, stringsAsFactors = F)[-1]
itRetA <- as.numeric(returnData$DE)
itRetB <- as.numeric(returnData$Cash)
itRetProb <- rep(1 / nrow(returnData), nrow(returnData))
...
save(data_vfi_t7, file = paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens4_3_v5.1.RData"))##
write.csv(data_vfi_t7, paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens4_3_v5.1.csv"), row.names = F)##


###########################################################################################################################################

#================H2 case sens4_4 (+3.684% ERP; better alpha scenario) =========================#
...
#importing historical return data
returnData <- read.csv("Ret_2_asset_sens4.csv", header = T, stringsAsFactors = F)[-1]
itRetA <- as.numeric(returnData$DE)
itRetB <- as.numeric(returnData$Cash)
itRetProb <- rep(1 / nrow(returnData), nrow(returnData))
...
save(data_vfi_t7, file = paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens4_4_v5.1.RData"))##
write.csv(data_vfi_t7, paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens4_4_v5.1.csv"), row.names = F)##

###########################################################################################################################################
#================H2 case sens5 (using vanguard benchmark) =========================#
...
save(data_vfi_t7, file = paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens5_v5.1.RData"))##
write.csv(data_vfi_t7, paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens5_v5.1.csv"), row.names = F)##

###########################################################################################################################################
#================H2 case sens6_1 (replacement rate = 0.6)=========================#
...
save(data_vfi_t7, file = paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens6_1_v5.1.RData"))##
write.csv(data_vfi_t7, paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens6_1_v5.1.csv"), row.names = F)##

###########################################################################################################################################

#================H2 case sens6_2 (replacement rate = 0.8) =========================#
...
save(data_vfi_t7, file = paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens6_2_v5.1.RData"))##
write.csv(data_vfi_t7, paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens6_2_v5.1.csv"), row.names = F)##

###########################################################################################################################################

#================H2 case sens7_1 (weight in target B = 0.5)=========================#
...
save(data_vfi_t7, file = paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens7_1_v5.1.RData"))##
write.csv(data_vfi_t7, paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens7_1_v5.1.csv"), row.names = F)##

###########################################################################################################################################
#================H2 case sens7_2 (weight in target B = 5)=========================#
...
save(data_vfi_t7, file = paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens7_2_v5.1.RData"))##
write.csv(data_vfi_t7, paste0("mgmt_H2_rho_",rho, "salary", salary, "_sens7_2_v5.1.csv"), row.names = F)##
