...
result_xx <- foreach(iter = 1:length(balances), .combine = rbind) %dopar% {
  ...
  #compute W_t
  set.seed(1)
  listOptim <- DEoptim(fn = temp_fun1_B , 
                       lower = c(LB_w),
                       upper = c(UB_w),
                       control = controlDE)
...
for (age_x in rev(age_start:(age_retire - 2))) {
  ...
  xdata_ord = xdata[order(xdata)]  # small to large
  ydata = resultsPrev$pseudoConsum
  ydata_ord = ydata[order(xdata)]  # small to large
  ...
  result_xx <- foreach(iter = 1:length(balances), .combine = rbind, .packages = "schumaker") %dopar% {
    ...
    #compute W_t
    set.seed(1)
    listOptim <- DEoptim(fn = temp_fun2_B , 
                         lower = c(LB_w),
                         upper = c(UB_w),
                         control = controlDE)
    optParam <- listOptim$optim$bestmem
...
save(data_vfi_t6, file = paste0("worker_B_case_rho",rho, "_salary", salary, "_base_v5.RData"))##
write.csv(data_vfi_t6, paste0("worker_B_case_rho",rho, "_salary", salary, "_base_v5.csv"), row.names = F)##

##############################################################################################################################################################

#================B case sens1_1 (salary = $55,000)=========================#
## Initialisation  
rm(list=ls())
...
salary <- 55000
rho <- 4
annuity1 <- 18.1313 # immediate life annuity at age 65 (sens1_1d on risk-free rate)
annuity2 <- 13.3559 # immediate life annuity at age 65 (sens1_1d on subjective discount factor)
beta <- 0.96 # subjective discount factor
limit1 <- 20500 # 401(k) contribution limit before threshold age
limit2 <- 27000 # 401(k) contribution limit on or after threshold age
LB_C1 <- salary - limit1 # lower bound for consumption before age 50
...
save(data_vfi_t6, file = paste0("worker_B_case_rho",rho, "_salary", salary, "_sens1_1_v5.RData"))##
write.csv(data_vfi_t6, paste0("worker_B_case_rho",rho, "_salary", salary, "_sens1_1_v5.csv"), row.names = F)##

##############################################################################################################################################################
#================B case sens1_2 (salary = $85,000)=========================#
## Initialisation  
rm(list=ls())
...
save(data_vfi_t6, file = paste0("worker_B_case_rho",rho, "_salary", salary, "_sens1_2_v5.RData"))##
write.csv(data_vfi_t6, paste0("worker_B_case_rho",rho, "_salary", salary, "_sens1_2_v5.csv"), row.names = F)##

##############################################################################################################################################################
#================B case sens2_1 (rho = 3)=========================#
## Initialisation  
rm(list=ls())
...
save(data_vfi_t6, file = paste0("worker_B_case_rho",rho, "_salary", salary, "_sens2_1_v5.RData"))##
write.csv(data_vfi_t6, paste0("worker_B_case_rho",rho, "_salary", salary, "_sens2_1_v5.csv"), row.names = F)##

##############################################################################################################################################################

#================B case sens2_2 (rho = 5)=========================#
## Initialisation  
rm(list=ls())
...
save(data_vfi_t6, file = paste0("worker_B_case_rho",rho, "_salary", salary, "_sens2_2_v5.RData"))##
write.csv(data_vfi_t6, paste0("worker_B_case_rho",rho, "_salary", salary, "_sens2_2_v5.csv"), row.names = F)##


##############################################################################################################################################################
#Note: Since sens 3_1 and sens 3_2 were found to be inappropriate for inclusion in our analysis, the relevant code has been omitted.

#================B Case sens4_1 (-1% ERP)=========================#
## Initialisation  
rm(list=ls())
...
save(data_vfi_t6, file = paste0("worker_B_case_rho",rho, "_salary", salary, "_sens4_1_v5.RData"))##
write.csv(data_vfi_t6, paste0("worker_B_case_rho",rho, "_salary", salary, "_sens4_1_v5.csv"), row.names = F)##

##############################################################################################################################################################


#================B case sens4_2 (+1% ERP)=========================#
## Initialisation  
rm(list=ls())
...
save(data_vfi_t6, file = paste0("worker_B_case_rho",rho, "_salary", salary, "_sens4_2_v5.RData"))##
write.csv(data_vfi_t6, paste0("worker_B_case_rho",rho, "_salary", salary, "_sens4_2_v5.csv"), row.names = F)##


##############################################################################################################################################################

#================B case sens5 (using investment benchmark from Vanguard)=========================#
## Initialisation  
rm(list=ls())
...
save(data_vfi_t6, file = paste0("worker_B_case_rho",rho, "_salary", salary, "_sens5_v5.RData"))##
write.csv(data_vfi_t6, paste0("worker_B_case_rho",rho, "_salary", salary, "_sens5_v5.csv"), row.names = F)##
